  ...
  <div class="authentication-inner row m-0">
    ...
    <div class="d-flex col-12 col-xl-4 align-items-center authentication-bg p-sm-12 p-6 overflow-hidden">
      <div class="w-px-400 mx-auto mt-0 pt-0 d-flex flex-column justify-content-center h-100">
        <h4 class="mb-1" style="color: #003d33; font-weight: 700;">Lupa Password? 🔒</h4>
        <p class="mb-6 text-muted">Masukkan email Anda dan kami akan mengirimkan instruksi untuk mengatur ulang password Anda.</p>
        @if (session('status'))
        <div class="mb-4 text-success fw-medium alert alert-success p-3" style="border-radius: 5px;">
          {{ session('status') }}
        </div>
        @endif
        <form id="formAuthentication" class="mb-6" action="{{ route('password.email') }}" method="POST">
          ...
          <div class="mb-6">
            ...
            <input type="text" class="form-control @error('email') is-invalid @enderror" id="email" name="email"
              placeholder="john@example.com" autofocus style="border-radius: 5px;" />
            @error('email')
          ...
          <button type="submit" class="btn btn-primary d-grid w-100" style="border-radius: 5px;">Kirim Link Reset</button>
        </form>
        <div class="text-center">
          @if (Route::has('login'))
          <a href="{{ route('login') }}" class="d-flex justify-content-center align-items-center" style="color: #003d33; font-weight: 600; text-decoration: none;">
            <i class="icon-base ti tabler-chevron-left scaleX-n1-rtl me-1.5"></i>
            Kembali ke login
          </a>
          @endif
        </div>
